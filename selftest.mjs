...
// [loop 746] TAM HÌNH phân loại + meaning — 4 loại có hậu quả khác nhau (vô ân/vô lễ/thế thế/tự hình).
//   Trước đây detectInteractions chỉ có name/vi (nhãn), KHÔNG có meaning (hậu quả).
{
  // 寅巳申 đủ 3 = pháp hình (heavy) + Vô ân meaning
  const _x = detectInteractions(fakePillars([['year','甲','寅'],['month','丁','巳'],['day','庚','申'],['time','戊','辰']]));
  assert(_x.xing.some((x) => x.name.includes('Vô ân') && x.meaning && x.meaning.includes('恩将仇报')), '[loop 746] Vô ân chi hình có meaning «恩将仇报»');
  assert(_x.xing.some((x) => x.heavy === true && x.vi === 'Pháp hình'), '[loop 746] 寅巳申 đủ 3 = pháp hình heavy');
  // 子卯 = Vô lễ meaning
  const _x2 = detectInteractions(fakePillars([['year','甲','子'],['month','丁','卯'],['day','戊','辰'],['time','戊','辰']]));
  assert(_x2.xing.some((x) => x.name === 'Vô lễ chi hình' && x.meaning && x.meaning.includes('luân lý')), '[loop 746] Vô lễ có meaning «luân lý»');
  // Tự hình meaning
  const _x3 = detectInteractions(fakePillars([['year','甲','辰'],['month','丙','辰'],['day','戊','戌'],['time','癸','亥']]));
  assert(_x3.xing.some((x) => x.name === 'Tự hình' && x.meaning && /tự hủy|trầm cảm|hoài nghi/.test(x.meaning)), '[loop 746] Tự hình có meaning «tự hủy/trầm cảm»');
  // Quân (戌-丑) = Thế thế + brief surface
  const _QR = analyze(1993, 10, 21, 1, 15, 'nam', 2026);
  assert(_QR.interactions.xing.some((x) => x.name === 'Thế thế chi hình' && x.meaning), '[loop 746] Quân có Hình thế thế + meaning');
  const { buildChartBrief } = await import('./src/engine/ai.js');
  assert(buildChartBrief(_QR).includes('TAM HÌNH'), '[loop 746] brief surface TAM HÌNH phân loại');
  console.log('   [loop 746] TAM HÌNH 4 loại (vô ân/vô lễ/thế thế/tự hình) + pháp hình + meaning ✓');
}
// [loop 747] LỤC HẠI 六害 — 6 cặp tên cổ + meaning (trước đây chỉ {a,b} không name/meaning).
{
  // 子未 = thế gia (cốt nhục)
  const _h = detectInteractions(fakePillars([['year','甲','子'],['month','丁','未'],['day','戊','辰'],['time','戊','午']]));
  const un = _h.hai.find((h) => (h.a === '子' && h.b === '未') || (h.a === '未' && h.b === '子'));
  assert(un && un.name === 'Thế gia tương hại' && un.meaning && /cốt nhục|mẹ-con|lục thân/.test(un.meaning), '[loop 747] 子未 hại = thế gia (cốt nhục) + meaning');
  // 酉戌 = đố kỵ
  const _h2 = detectInteractions(fakePillars([['year','辛','酉'],['month','壬','戌'],['day','戊','辰'],['time','戊','午']]));
  const yx = _h2.hai.find((h) => (h.a === '酉' && h.b === '戌') || (h.a === '戌' && h.b === '酉'));
  assert(yx && yx.name === 'Đố kỵ tương hại', '[loop 747] 酉戌 hại = đố kỵ');
  // Quân (酉-戌 năm-tháng) surface trong brief
  const _QR = analyze(1993, 10, 21, 1, 15, 'nam', 2026);
  assert(_QR.interactions.hai.some((h) => h.vi === 'Hại đố kỵ' && h.meaning), '[loop 747] Quân 酉-戌 hại = đố kỵ + meaning');
  const { buildChartBrief } = await import('./src/engine/ai.js');
  assert(buildChartBrief(_QR).includes('LỤC HẠI'), '[loop 747] brief surface LỤC HẠI phân loại');
  console.log('   [loop 747] LỤC HẠI 6 cặp tên cổ + meaning (thế gia/quan quỷ/vô ân/đồng bào/tranh tiến/đố kỵ) ✓');
}
// [loop 748] LỤC XUNG 六冲 — 6 cặp ảnh hưởng LĨNH VỰC khác nhau (trước đây chỉ {a,b}).
{
  // 卯酉 = đào hoa (nam nữ/hôn nhân)
  const _c = detectInteractions(fakePillars([['year','甲','卯'],['month','丁','酉'],['day','戊','辰'],['time','戊','午']]));
  const my = _c.chong.find((c) => (c.a === '卯' && c.b === '酉') || (c.a === '酉' && c.b === '卯'));
  assert(my && my.name === 'Đào hoa tương xung' && my.domain === 'nam nữ/hôn nhân' && my.meaning, '[loop 748] 卯酉 xung = đào hoa [nam nữ/hôn nhân] + meaning');
  // 子午 = thủy hỏa
  const _c2 = detectInteractions(fakePillars([['year','甲','子'],['month','丁','午'],['day','戊','辰'],['time','戊','申']]));
  assert(_c2.chong.some((c) => c.name === 'Thủy hỏa tương chiến' && c.domain === 'tâm huyết/thần kinh'), '[loop 748] 子午 xung = thủy hỏa [tâm huyết]');
  // Mỹ Anh (亥-巳) = thiên môn [công việc]
  const _MR = analyze(1996, 12, 4, 10, 15, 'nữ', 2026);
  assert(_MR.interactions.chong.some((c) => c.name === 'Thiên môn địa hộ' && c.domain === 'công việc/giao tế'), '[loop 748] Mỹ Anh 亥-巳 xung = thiên môn [công việc]');
  const { buildChartBrief } = await import('./src/engine/ai.js');
  assert(buildChartBrief(_MR).includes('LỤC XUNG'), '[loop 748] brief surface LỤC XUNG phân loại');
  console.log('   [loop 748] LỤC XUNG 6 cặp + LĨNH VỰC (thủy hỏa/trọng thổ/dịch mã/đào hoa/la võng/thiên môn) ✓');
}
// [loop 749] BÁN HỢP 半合 strength — sinh+vượng mạnh, vượng+mộ trung, củng(拱) yếu.
{
  // 申辰 missing 子(帝旺) = 拱 (yếu nhất)
  const _g = detectInteractions(fakePillars([['year','甲','申'],['month','丁','卯'],['day','戊','辰'],['time','戊','午']]));
  const gong = _g.banHe.find((b) => b.present.includes('申') && b.present.includes('辰'));
  assert(gong && gong.banType === 'Củng hợp (拱)' && gong.strength === 'yếu' && gong.missingRole.includes('帝旺'), '[loop 749] 申辰 thiếu 子 = Củng hợp拱 (yếu, thiếu 帝旺)');
  // 申子 missing 辰(墓) = sinh vượng (mạnh nhất)
  const _sv = detectInteractions(fakePillars([['year','甲','申'],['month','丁','子'],['day','戊','午'],['time','戊','卯']]));
  const sv = _sv.banHe.find((b) => b.present.includes('申') && b.present.includes('子'));
  assert(sv && sv.banType === 'Sinh vượng bán hợp' && sv.strength === 'mạnh', '[loop 749] 申子 thiếu 辰 = sinh vượng (mạnh nhất)');
  // Quân 酉丑 missing 巳 = vượng mộ (trung)
  const _QR = analyze(1993, 10, 21, 1, 15, 'nam', 2026);
  const qban = _QR.interactions.banHe.find((b) => b.present.includes('酉') && b.present.includes('丑'));
  assert(qban && qban.strength === 'trung' && qban.missingRole.includes('长生'), '[loop 749] Quân 酉丑 thiếu 巳 = vượng mộ (trung)');
  const { buildChartBrief } = await import('./src/engine/ai.js');
  assert(buildChartBrief(_QR).includes('BÁN HỢP'), '[loop 749] brief surface BÁN HỢP strength');
  console.log('   [loop 749] BÁN HỢP 3 strength (sinh vượng mạnh / vượng mộ trung / củng拱 yếu) ✓');
}
// [loop 752] LỤC HỢP 六合 合化 vs 合绊 — 得时 (tháng chi bản khí == hóa hành) → 合化; else 合绊.
{
  // 合化: month 申(金) + 辰酉 → hóa金 đắc lệnh
  const _hua = detectInteractions(fakePillars([['year','甲','辰'],['month','丁','申'],['day','戊','酉'],['time','戊','午']]));
  const _h1 = _hua.zhiHe.find((h) => (h.a === '辰' && h.b === '酉') || (h.a === '酉' && h.b === '辰'));
  assert(_h1 && _h1.heType === '合化' && _h1.huaNote.includes('ĐỔI ngũ hành'), '[loop 752] 辰酉 + month申(金) = 合化 (đắc lệnh → đổi ngũ hành)');
  // 合绊: month 卯(木) + 辰酉 → không đắc lệnh
  const _ban = detectInteractions(fakePillars([['year','甲','辰'],['month','丁','卯'],['day','戊','酉'],['time','戊','午']]));
  const _h2 = _ban.zhiHe.find((h) => (h.a === '辰' && h.b === '酉') || (h.a === '酉' && h.b === '辰'));
  assert(_h2 && _h2.heType === '合绊' && _h2.huaNote.includes('DÍNH'), '[loop 752] 辰酉 + month卯(木) = 合绊 (dính, không hóa)');
  // Bố (未+午) có heType
  const _BR = analyze(1964, 4, 4, 12, 0, 'nam', 2026);
  assert(_BR.interactions.zhiHe.every((h) => h.heType), '[loop 752] mọi zhiHe có heType (合化/合绊)');
  console.log('   [loop 752] LỤC HỢP 合化 (đắc lệnh đổi ngũ hành) vs 合绊 (dính không hóa) ✓');
}
...
// [loop 1192→1330] 滴天髓 «通論» — 22 nguyên lý (任鐵樵闡微 ctext/quanxue.cn/wikisource).
assert(Object.keys(DITIANSUI_TONGLUN).length === 22, `DITIANSUI_TONGLUN đủ 22 nguyên lý (got ${Object.keys(DITIANSUI_TONGLUN).length})`);
...
assert(DITIANSUI_TONGLUN['贞元'].verse.includes('造化起於元') && DITIANSUI_TONGLUN['配合'].verse.includes('配合干支'), '通論 [loop 1328]: 贞元(第29章) + 配合(第七章) verse');
assert(DITIANSUI_TONGLUN['形象'].verse.includes('兩氣合而成象') && DITIANSUI_TONGLUN['方局'].verse.includes('方是方兮局是局'), '通論 [loop 1330]: 形象(第10章) + 方局(第11章) verse');
...
// [loop 1342] regression guard — buildChartBrief phải surface đủ block đã wire (loop 1326-1341)
{
  const { buildChartBrief } = await import('./src/engine/ai.js');
  const _GR = analyze(1990, 6, 15, 8, 30, 'male', 2026);
  const _GB = buildChartBrief(_GR);
  const _h = (s) => _GB.includes(s);
  assert(_h('«通論»') && _h('«何知章»') && _h('«十神 chuyên luận»') && _h('«十神 bản chất»'), '[loop 1342] brief có cụm tri thức 滴天髓 + 十神');
  assert(_h('«Lĩnh vực cuộc sống»') && _h('«Cách cục hướng dẫn»') && _h('«Tương tác can chi»') && _h('«源流»') && _h('«Chất lượng trụ»'), '[loop 1342] brief có 5 block wired (LIFE_AREA/PATTERN_GUIDE/INTERACTION/yuanliu/pillarQuality)');
  assert(/Thiên Ất|Tướng Tinh|Thiên Đức|Kim Dư/.test(_GB), '[loop 1342] brief THẦN SÁT dùng tên Việt (không pinyin thô)');
...
// [loop 1208] 大象传 (周易) — 5 quẻ nền tảng, verse Hán gốc (Wikisource 周易/大象).
{
  ...
  assert(DAXIANG['大畜'].verse.includes('天在山中') && DAXIANG['咸'].verse.includes('山上有泽') && DAXIANG['大过'].verse.includes('泽灭木'), '大象 [loop 1211]: 大畜=天在山中, 咸=山上有泽, 大过=泽灭木');
...
// [loop 1194] 神煞 đối chiếu 三命通會/渊海子平 — regression guard các core sao.
{
  const { TIAN_YI, WEN_CHANG, TAO_HUA, YI_MA, JIANG_XING, HUA_GAI, TIAN_DE, YUE_DE, JIN_YU, SHENSHA_KOUJUE } = await import('./src/engine/shensha.js');
  assert(SHENSHA_KOUJUE && SHENSHA_KOUJUE['天德'].includes('坤'), 'SHENSHA_KOUJUE: 天德 khẩu quyết');
  assert(TIAN_YI['甲'].join() === '丑,未' && TIAN_YI['辛'].join() === '寅,午', '天乙: 甲戊庚牛羊, 六辛逢马虎');
  assert(WEN_CHANG['庚'] === '亥' && WEN_CHANG['辛'] === '子' && WEN_CHANG['壬'] === '寅', '文昌: 庚猪辛鼠壬逢虎 (loop 549 fix 辛→子)');
  assert(TAO_HUA['B'] === '卯' && YI_MA['B'] === '申' && JIANG_XING['B'] === '午' && HUA_GAI['B'] === '戌', '寅午戌: 桃花卯/驿马申/将星午/华盖戌');
  assert(TIAN_DE['午'] === '亥' && TIAN_DE['酉'] === '寅' && TIAN_DE['子'] === '巳', '天德: 五月乾(亥)八月艮(寅)子巽(巳)');
  assert(YUE_DE['B'] === '丙' && YUE_DE['C'] === '庚' && YUE_DE['A'] === '壬', '月德: 寅午戌丙/巳酉丑庚/申子辰壬');
...
// [loop 1276] 阳干 12-stage full verify (长生=学堂, 临官=禄, 帝旺=羊刃).
{
  const { CHANGSHENG_START, ZHI_ORDER } = await import('./src/engine/constants.js');
  const { LU_SHEN, YANG_REN, XUE_TANG } = await import('./src/engine/shensha.js');
  for (const g of ['甲','丙','戊','庚','壬']) {
    const si = ZHI_ORDER.indexOf(CHANGSHENG_START[g]);
    assert(ZHI_ORDER[si % 12] === XUE_TANG[g] && ZHI_ORDER[(si + 3) % 12] === LU_SHEN[g] && ZHI_ORDER[(si + 4) % 12] === YANG_REN[g], `${g}阳干 12-stage: 长生=学堂/临官=禄/帝旺=羊刃`);
  }
...
//   Trước đây 5 tool bói (analyze_char/meihua/liuren/qimen/guiguzi) FLAT → Z.ai API reject → AI không bao giờ gọi được.
{
  ...
  assert(AI_TOOLS.length === 37, `[loop 623→R42] AI_TOOLS có đúng 37 tool (got ${AI_TOOLS.length})`);
...
// [loop 1089] 门迫 (宫克吉门) — 凶格 mới. Quét 12 giờ, ít nhất 1 lúc 吉门 bị cung khắc.
{
  let _menPoCount = 0, _sample = null;
  for (let h = 0; h < 12; h++) {
    const r = qimenDongPan(2026, 6, 21, h);
    const mp = (r.xiongGe || []).find((x) => x.name === '门迫');
    if (mp) { _menPoCount++; if (!_sample) _sample = mp; }
  }
  assert(_menPoCount > 0, `[loop 1089] 门迫 (宫克吉门) phát hiện trong 12 giờ (${_menPoCount}/12)`);
  assert(_sample && /cung khắc门|Môn Phách/.test(_sample.detail || _sample.note || ''), `[loop 1089] 门迫 detail đúng (got "${_sample?.detail}")`);
  // deterministic rule check: 开门(Kim) ở 离9(Hỏa) → Hỏa khắc Kim = 门迫 (KE[火]=金)
  //   verify via the same logic: cung9 克 开门 — engine phải flag nếu 开 ở 9
  console.log(`   [loop 1089] 门迫 ✓ — 凶格 «吉门受宫克为迫» (${_menPoCount}/12 giờ, vd ${_sample?.detail || _sample?.vi})`);
}
...
const _validTones = new Set(['cat', 'hung', 'mixed', 'neutral']);
for (const e of by.events) {
  ...
  assert(_validTones.has(e.tone), `event ${e.year} có tone hợp lệ («${e.tone}»)`);
}
...
// [loop 994] TONE — 2036 mở kho bật Tài(=Dụng của spR 乙木) → CÁT; 2026 bật Ấn(=仇, sinh Kỵ Tỷ) → HUNG
assert(by2036z && by2036z.tone === 'cat', '[994] 2036 bật nhóm Tài = Dụng → tone CÁT');
assert(by.catYears.includes(2036), '[994] catYears chứa 2036');
        ...
        for (const _h of [0, 12, 23]) {
          ...
          try {
            ...
            for (const e of _o.events) {
              if (!(e.year >= 2026 && e.year <= 2037 && e.groups.length > 0 && _validTypes.has(e.type) && _validGrades.has(e.grade) && _validTones.has(e.tone) && e.groups.every((g) => _validGroups.has(g.group)))) _byCrash++;
            }
          } catch (e) { _byCrash++; }
        }
      }
...
// [loop 995] 应期 integration vào analyzeLiunianDeep — năm kích hoạt có out.yingqi
const lnC2036 = analyzeLiunianDeep(spR, 2036, spR.patternQuality?.patternYong);
assert(lnC2036.yingqi && lnC2036.yingqi.some((e) => e.type === 'xung mở kho' && e.tone === 'cat'), '[995] 2036 = năm mở kho (CÁT) → analyzeLiunianDeep.out.yingqi có event');
assert(!analyzeLiunianDeep(spR, 2029, spR.patternQuality?.patternYong).yingqi, '[995] 2029 không xung/hợp chi → KHÔNG có out.yingqi');
...
{
  const { AI_TOOLS } = await import('./src/engine/ai.js');
  assert(AI_TOOLS.length === 37, `[loop 792→R49] 37 tools (got ${AI_TOOLS.length})`);
  let _noDesc = 0;
...
// [loop 837] pDaily performance — <200ms (includes 6 evaluateDate for 宜忌).
{
  const _QR = analyze(1993, 10, 21, 1, 15, 'nam', 2026);
  const _t0 = Date.now();
  for (let _i = 0; _i < 5; _i++) composeAnswer('hôm nay nên làm gì?', _QR);
  const _avg = (Date.now() - _t0) / 5;
...
// [loop 1032] 称骨 MONTH/DAY/HOUR weight — 三命通会 chuẩn + end-to-end
{
  const { MONTH_WEIGHT, DAY_WEIGHT, HOUR_WEIGHT, chenggu } = await import('./src/engine/chenggu.js');
  const { analyze } = await import('./src/engine/chart.js');
  const _M = [0.6,0.7,1.8,0.9,0.5,1.6,0.9,1.5,1.8,0.8,0.9,0.5];
  const _D = [0.5,1.0,0.8,1.5,1.6,1.5,0.8,1.6,0.8,1.6,0.9,1.7,0.8,1.7,1.0,0.8,0.9,1.8,0.5,1.5,1.0,0.9,0.8,0.9,1.5,1.8,0.7,0.8,1.6,0.6];
  const _H = { 子:1.6,丑:0.6,寅:0.7,卯:1.0,辰:0.9,巳:1.6,午:1.0,未:0.8,申:0.8,酉:0.9,戌:0.6,亥:0.6 };
  assert(MONTH_WEIGHT.every((v, i) => v === _M[i]), '[loop 1032] MONTH_WEIGHT 三命通会 12/12');
  assert(DAY_WEIGHT.every((v, i) => v === _D[i]), '[loop 1032] DAY_WEIGHT 三命通会 30/30');
  assert(Object.keys(_H).every((k) => HOUR_WEIGHT[k] === _H[k]), '[loop 1032] HOUR_WEIGHT 三命通会 12/12');
  // end-to-end: 1990-3-15 10:30 → 庚午0.9+二月0.7+十九0.5+巳1.6 = 3.7 (ba-zi.ai example)
  const _R = analyze(1990, 3, 15, 10, 30, 'nam', 2026);
  const _cg = chenggu(_R);
  assert(Math.abs(_cg.totalLiang - 3.7) < 0.001, `[loop 1032] chenggu(1990-3-15 10:30) = 3.7 两 (got ${_cg.totalLiang})`);
  assert(/运不通|空/.test(_cg.verse || ''), '[loop 1032] 3.7 两 verse hiển thị');
  console.log(`   [loop 1032] 称骨 MONTH/DAY/HOUR (三命通会) + end-to-end 3.7 两 verified ✓`);
}
// Tất định
...
// ################## [loop 31] destiny-timeline golden-years rank≤3 + empty dayun guard ##################
{
  ...
  // [loop 1002] 应期 integration — mỗi thập kỷ có yingqiActivations (nếu đại vận chi xung/hợp)
  const tlYq = lifeTimeline(analyze(1993, 10, 21, 0, 30, 'nam', 2026));
  const dc55 = tlYq.decades.find((d) => d.ganZhi === '丙辰');
  assert(dc55 && dc55.yingqiActivations && dc55.yingqiActivations.some((a) => a.type === 'xung mở kho' && a.tone === 'cat'), '[1002] timeline 丙辰 decade có 应期 ★MởKho (CÁT)');
}
...
import { computeHehun as _hehun } from './src/engine/hehun.js';
{
  ...
  let _pairs = 0, _crash = 0, _falseKe = 0, _ganKeN = 0, _tkdcN = 0, _maxF = 0, _maxFex = '';
  ...
  for (let _i = 0; _i < 60; _i++) {
    ...
    // [loop 927] track factor max-length — tool AI truncate _s(f,150), factor dài bị cắt mất khuyên
    for (const _f of _h.factors) if (_f.length > _maxF) { _maxF = _f.length; _maxFex = _f.slice(0, 50); }
  ...
  assert(_maxF <= 150, `[loop 927] hehun factor ≤150 ký tự (tool AI truncate _s(f,150)) — max ${_maxF}: «${_maxFex}…»`);
  console.log(`   [loop 926] HEHUN ✓ — ${_pairs} cặp, 日干相克×${_ganKeN}, 天克地冲×${_tkdcN}, 0 false; factor-max ${_maxF} ký tự`);
}
...
// [loop 708] 17-TOOL SMOKE TEST — tất cả tools phải trả data (không error) với valid params
{
  ...
  const _tests = [
    ...
    ['health_today', {}], ['evaluate_number', {number:'0912345678'}],
  ...
  assert(_ok === 28, `[loop 708→R42] 28/28 tools pass smoke test (got ${_ok}/28)`);
  console.log(`   28-TOOL SMOKE ✓ — ${_ok}/28 tools trả data với valid params`);
}
...
// [loop 1014] zheri 六合 reward — ngày chi lục hợp tuổi → +15 (đóng bug-class loop 1006 sót zheri)
{
  const { evaluateDate } = await import('./src/engine/zheri.js');
  const { Solar } = await import('lunar-javascript');
  // tìm ngày 丑 trong 2026-6 → user=子 → 六合
  let found = false;
  for (let d = 1; d <= 30 && !found; d++) {
    const dz = Solar.fromYmdHms(2026, 6, d, 12, 0, 0).getLunar().getDayZhi();
    if (dz === '丑') {
      const rUser = evaluateDate(2026, 6, d, 'marry', '子');     // 六合 丑-子
      const rNone = evaluateDate(2026, 6, d, 'marry', null);      // không user → không reward
      assert(rUser.score - rNone.score === 15, `[loop 1014] 六合 reward = +15 (got delta ${rUser.score - rNone.score}, 丑日 user=子)`);
      assert(rUser.reasons.some((x) => /LỤC HỢP/.test(x)), '[loop 1014] 六合 reason hiển thị');
      found = true;
    }
  }
  assert(found, '[loop 1014] tìm được ngày 丑 trong 2026-6 để test');
  // 冲 ngày vẫn phạt (không bị 六合 ghi đè): user=午, tìm ngày 子(冲午) → clashYou + phạt
  for (let d = 1; d <= 30; d++) {
    const dz = Solar.fromYmdHms(2026, 6, d, 12, 0, 0).getLunar().getDayZhi();
    if (dz === '子') {
      const rChong = evaluateDate(2026, 6, d, 'marry', '午');
      assert(rChong.clashYou === true && rChong.score < 50, `[loop 1014] 冲 ngày vẫn phạt (子日 user=午, score=${rChong.score})`);
      break;
    }
  }
...
// [loop 1022→1053] đông-y CONDITION_KB — 27 bệnh, keyword match chính xác + không garbled
{
  ...
  console.log(`   [loop 1053] đông-y KB (33 conditions, match ${_ok}/${_cases.length}, clean) ✓`);
}
...
// ============================================================================
{
  ...
  const _allMvi = ly.months.every((m) => m.mVi && typeof m.solarMonthVi === 'string' && m.solarMonthVi.includes('dương'));
  assert(_allMvi, `[liuyue] mỗi tháng có mVi + solarMonthVi dương`);
