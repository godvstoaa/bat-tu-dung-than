...
- 生肖配对 (con giáp hợp - khắc): module tương tác — user nhập tuổi 2 người (chi năm sinh) → điểm 0-100 + bóc tách Lục Hợp / Tam Hợp / Lục Xung / Lục Hại / Tam Hình / Tự Hình.${(() => { try { const zp = zodiacPairScore('子', '午'); return ` [kiểm tra dữ liệu] 生肖配对 sẵn sàng: mẫu Tý×Ngọ = ${zp.score}/100 (${zp.rating}) — ${zp.relations.map((r) => r.name).join('+') || 'không phạm'}.`; } catch (e) { return ' [kiểm tra dữ liệu] 生肖配对: (không tính được)'; } })()}`;
  ...
  try {
    ...
    brief += "\n--- KIẾN THỨC CỔ PHÁP MỞ RỘNG (crawled) ---\n" +
      ...
      "滴天髓 ĐÁNH GIÁ CỤ THỂ (任铁樵评注 pattern match): " + (() => { try { const dm = c.dayMaster.gan; const matches = (DITIANSUI_PATTERNS || []).filter(p => p.pattern && p.pattern.includes(dm)).slice(0, 3).map(p => p.reading.slice(0, 100)); return matches.length ? matches.join(' | ') : '(không match)'; } catch (e) { return '(không load)'; } })() + "\n" +
      "鬼谷子算命 DAY PILLAR: " + (() => { try { const dp = c.pillars.day.gan + c.pillars.day.zhi; const g = GUIGU_60JIAZI?.[dp]; return g ? `${dp} 「${g.title || ''}」→ ${g.reading.slice(0, 120)}` : '(không có)'; } catch (e) { return '(không load)'; } })() + "\n" +
      ...
      "3 CLASSICS DEEP (filter theo cách cục " + (R.pattern?.name || '?') + "): " + (() => {
        try {
          const pname = R.pattern?.name || '';
          const gods = new Set();
          for (const pos of ['year','month','day','time']) { const pl = c.pillars[pos]; if (pl.ganGod) gods.add(pl.ganGod); (pl.hidden||[]).forEach(h => { if (h.god) gods.add(h.god); }); }
          const godStr = [...gods].join(' ');
          const relevant = (ZIPING_CLASSICS_DEEP || []).filter(e => {
            const t = (e.rule || '') + (e.application || '');
            return (pname && t.includes(pname.replace('格',''))) || [...gods].some(g => t.includes(g)) || t.includes('用神') || t.includes('格局') || t.includes('调候');
          }).slice(0, 5);
          return relevant.length ? relevant.map(e => '«' + e.rule.slice(0, 80) + '»').join(' | ') : '(filter theo ' + pname + ' — 67 rules available)';
        } catch (e) { return '(67 rules available)'; }
      })() + "\n" +
  ...
  // === [WESTERN] Framework đối chiếu (chiêm tinh phương Tây) ===
  try {
...
export const AI_TOOLS = [
  ...
  { type: 'function', function: {
    name: 'analyze_western', description: 'BẢN ĐỒ SAO PHƯƠNG TÂY (Western astrology — tropical zodiac): tính từ cùng ngày/giờ sinh của user → cung Mặt Trời (Sun), Mặt Trăng (Moon), Mercury/Venus/Mars/Jupiter/Saturn/Uranus/Neptune/Pluto, Ascendant (Rising), MC, 12 cung (houses), aspects, cân bằng nguyên tố. Dùng khi user hỏi «chiêm tinh phương Tây/bản đồ sao/cung hoàng đạo/Sun sign/Moon/Rising» HOẶC muốn «so sánh/đối chiếu» Bát Tự với Western. Trả chart + interpretation (SUN/MOON/ASC readings) + BAZI_WESTERN_MAP. Args lat/lng (toạ độ sinh, mặc định Hà Nội 21.03,105.85 — Ascendant nhạy địa điểm, hãy hỏi user «sinh ở đâu» nếu cần chính xác). KHÁC Bát Tự: Western dùng 10 hành tinh + aspects + houses; BaZi dùng 5 hành + thập thần. Cả 2 đều tham khảo, KHÔNG khoa học.',
    parameters: { type: 'object', properties: {
      lat: { type: 'number', description: 'Vĩ độ nơi sinh (độ, +Bắc). Mặc định 21.03 (Hà Nội).' },
  ...
  { type: 'function', function: {
    name: 'analyze_tarot', description: 'TAROT — rút 3 (hoặc N) lá Major Arcana Rider-Waite (Quá khứ-Hiện tại-Tương lai) kèm luận upright/reversed. Dùng khi user hỏi «rút tarot/rút tarot/xem bài tây phương Tây». Args: count (1-5, mặc định 3). Truyền thống Tây, tham khảo — không định mệnh.',
  ...
  { type: 'function', function: {
    name: 'analyze_rune', description: 'RUNES — rút 1-3 rune cổ Bắc Âu (Elder Futhark, 24 rune) kèm ý nghĩa. Dùng khi user hỏi «rút rune/rune casting/phép rune Bắc Âu». Args: count (1-3, mặc định 1).',
    parameters: { type: 'object', properties: { count: { type: 'number', description: 'Số rune (1-3, mặc định 1).' } }, required: [] },
  } },
  { type: 'function', function: {
    name: 'analyze_iching', description: 'KINH DỊCH — rút 1 quẻ trong 64 quẻ cho câu hỏi hiện tại. Dùng khi user hỏi «xem quẻ kinh dịch/xem quẻ Dịch/lấy 1 quẻ». (App đã có 河洛+鬼谷卦 tool khác — tool này cho quẻ đơn giản nhanh.)',
    parameters: { type: 'object', properties: {}, required: [] },
  } },
  { type: 'function', function: {
    name: 'analyze_coffee', description: 'ĐỌC BÃ CÀ PHÊ/TRÀ — 3 ký hiệu ngẫu nhiên + ý nghĩa. Dùng khi user hỏi «đọc bã cà phê/đọc bã cà phê/đọc lá trà». Truyền thống Thổ Nhĩ Kỳ/Phương Đông.',
...
export function execTool(name, args, R) {
  ...
  try {
    switch (name) {
      ...
      case 'analyze_western': { // [WESTERN] bản đồ sao phương Tây — đối chiếu với Bát Tự
        try {
          const inp = R.chart?.input || {};
          const lat = Number(a.lat) || 21.03;   // mặc định Hà Nội
          const lng = Number(a.lng) || 105.85;
          // Local solar time (Bát Tự) → UT (cho astronomy-engine): UT = local − lng/15 giờ
          const baseDay = new Date(Date.UTC(inp.year || 1990, (inp.month || 1) - 1, inp.day || 15, 0, 0, 0));
          const utOffsetMin = -(lng / 15) * 60;
          baseDay.setUTCMinutes(baseDay.getUTCMinutes() + ((inp.hour || 12) * 60 + (inp.minute || 0)) + utOffsetMin);
          const wc = computeWesternChart(baseDay, lat, lng);
          // Interpretation từ KB
          const interp = {
            sunReading: SUN_IN_SIGN[wc.sun.signVi] || '',
            moonReading: MOON_IN_SIGN[wc.moon.signVi] || '',
            risingReading: ASCENDANT_MEANING[wc.ascendant.signVi] || '',
            bigThree: wc.summary.bigThree,
            elementBalance: wc.elementBalance,
            topAspects: wc.aspects.filter(x => /tam hợp|đối xung|vuông góc|hợp nhất/.test(x.aspect)).slice(0, 6),
          };
          // So sánh BaZi ↔ Western
