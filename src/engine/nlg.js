...
function pTiming(R, intent) {
  ...
  try { const by = scanBranchYingqi(R, new Date().getFullYear(), 12); if (by.events.length) lines.push(`🔓 Xung/Hợp kích hoạt (sao ẩn bật ra): ${by.events.slice(0, 4).map((e) => `${e.year}(${e.yz}) ${e.type === 'xung mở kho' ? '★MởKho' : e.type === 'tam hợp thành cục' ? '≡TamHợp' : e.type === 'hợp dẫn' ? '∼Hợp' : '×Xung'}→${e.groups.map((g) => g.vi).join('+')}${e.grade === 'zhen' ? ' ✦真应期' : e.grade === 'zu' ? ' ⚠giảmLực' : ''}`).join('; ')}.${by.zhenYears.length ? ` ✦真应期 (đại vận cùng hướng — phát thật): ${by.zhenYears.join(', ')}.` : ''}`); } catch (e) {}
