...
function pTiming(R, intent) {
  ...
  // [loop 989] 应期 «NỬA CHI» — sao ẨN trong địa chi bẩm sinh kích hoạt khi lưu niên XUNG (mở kho) / HỢP (kéo ra).
  //   Bù cho nửa «CAN» (thấu can) ở dòng trên: lá số có sao tàng căn thì chỉ xung/hợp chi mới «bật» sao ra.
  try { const by = scanBranchYingqi(R, new Date().getFullYear(), 12); if (by.events.length) lines.push(`🔓 Xung/Hợp kích hoạt (sao ẩn bật ra): ${by.events.slice(0, 4).map((e) => `${e.year}(${e.yz}) ${e.type === 'xung mở kho' ? '★MởKho' : e.type === 'tam hợp thành cục' ? '≡TamHợp' : e.type === 'hợp dẫn' ? '∼Hợp' : '×Xung'}→${e.groups.map((g) => g.vi).join('+')}`).join('; ')}.${by.kuBranches.length ? ` Lá số có kho ${by.kuBranches.map((k) => k.zhi).join('/')} → năm xung mở kho là «bật kho» mạnh nhất.` : ''}`); } catch (e) {}
