...
function pTiming(R, intent) {
  ...
  // [loop 999] badDy: top-2 âm, nhưng LUÔN gồm dayun clashMitigated (天克地冲+Dụng) —
  //   nuance «đổi thay có lợi» quan trọng, không được cắt vì slice(0,2).
  const _badAll = dy.filter((d) => d.score < 0);
  const badDy = [];
  for (const d of _badAll.slice(0, 2)) badDy.push(d);
  for (const d of _badAll) if (d.clashMitigated && !badDy.includes(d)) badDy.push(d);
  if (goodDy.length) lines.push(`Đại vận CÁT: ${goodDy.map((d) => `${hanviet(d.ganZhi)} [${d.startAge}–${d.startAge + 9}t]${d.gejuRescue ? ' ★RESCUES' : d.gejuDelta > 0 ? ' ★CÁCH THUẬN' : ''}${d.clashType === '天克地冲' ? (d.clashMitigated ? ' ⚡đổithayCÓLỢI' : ' ⚡biếnloạn') : d.clashType === '地冲' ? ' ⚡biếnđộng' : d.clashType === '伏吟' ? ' ⚡trùphúc' : ''}`).join('; ')} — tiến thủ.`);
  if (badDy.length) lines.push(`Đại vận cần THẬN TRỌNG: ${badDy.map((d) => `${hanviet(d.ganZhi)} [${d.startAge}–${d.startAge + 9}t]${d.gejuWorsen ? ' ⚠WORSENS' : d.gejuDelta < 0 ? ' ⚠CÁCH NGHỊCH' : ''}${d.clashType === '天克地冲' ? (d.clashMitigated ? ' ⚡nhưngMangDụng→đổithayCÓLỢI(chủđộngnắm)' : ' ⚡天克地冲đạiLoạn') : d.clashType === '地冲' ? ' ⚡地冲biếnđộng' : d.clashType === '伏吟' ? ' ⚡伏吟trùphúc' : ''}`).join('; ')} — giữ ổn định.`);
  ...
  // [loop 989/990] 应期 «NỬA CHI» + 大运 grading — sao ẨN trong địa chi kích hoạt khi lưu niên XUNG/HỢP;
  //   [990] mỗi năm được grade theo đại vận: ★真应期 (đại vận cùng hướng → phát thật) / ⚠ giảm lực (kỵ vận).
  try { const by = scanBranchYingqi(R, new Date().getFullYear(), 12); if (by.events.length) lines.push(`🔓 Xung/Hợp kích hoạt (sao ẩn bật ra): ${by.events.slice(0, 4).map((e) => `${e.year}(${e.yz}) ${e.type === 'xung mở kho' ? '★MởKho' : e.type === 'tam hợp thành cục' ? '≡TamHợp' : e.type === 'hợp dẫn' ? '∼Hợp' : '×Xung'}→${e.groups.map((g) => g.vi).join('+')}${e.grade === 'zhen' ? ' ✦真应期' : e.grade === 'zu' ? ' ⚠giảmLực' : ''}${e.tone === 'cat' ? ' 🎉Dụng' : e.tone === 'hung' ? ' ⚠Kỵ' : e.tone === 'mixed' ? ' ≡Dụng+Kỵ' : ''}`).join('; ')}.${by.catYears.length ? ` 🎉Bật DỤNG sao (CÁT): ${by.catYears.join(', ')}.` : ''}${by.hungYears.length ? ` ⚠Bật KỴ sao (HUNG): ${by.hungYears.join(', ')}.` : ''}`); } catch (e) {}
  // [loop 997] 大运 chi activation — thập kỷ nào «mở cửa» sao ẩn (cả 10 năm, mạnh hơn 1 năm)
  try { const by2 = scanBranchYingqi(R, new Date().getFullYear(), 12); if (by2.dayunActivations && by2.dayunActivations.length) lines.push(`🔄 Đại vận kích hoạt (cả thập kỷ): ${by2.dayunActivations.slice(0, 3).map((d) => `${d.ganZhi}[${d.startAge}–${d.startAge + 9}t] ${d.type === 'xung mở kho' ? '★MởKho' : d.type === 'tam hợp thành cục' ? '≡TamHợp' : d.type === 'hợp dẫn' ? '∼Hợp' : '×Xung'}→${d.groups.map((g) => g.vi).join('+')}${d.tone === 'cat' ? ' 🎉Dụng' : d.tone === 'hung' ? ' ⚠Kỵ' : ''}`).join('; ')}.`); } catch (e) {}
...
function pDaily(R, intent) {
  ...
  try {
    ...
    paras.push(`📋 ${_dayLabel.charAt(0).toUpperCase() + _dayLabel.slice(1)} 宜 (nên): ${_yi.join(', ') || '(không có việc đặc biệt宜)'}. 忌 (kỵ): ${_ji.join(', ') || '(không)'}.`);
  } catch (e) {}
...
function pBestDays(R, intent) {
  ...
  if (/\b(cuoi|cuoi hoi|hon le|cuoi xon|wedding|marry|marriage)\b/.test(_q)) { _actId = 'marry'; _actLabel = 'Cưới hỏi'; }
  else if (/\b(khai truong|mo cua|kinh doanh|buon ban|business|grand opening|open shop)\b/.test(_q)) { _actId = 'business'; _actLabel = 'Khai trương'; }
  else if (/\b(dong tho|xay dung|xay nha|xay cua|dip nha|nhap trach|don nha|construction|build house|renovat|move house|relocat)\b/.test(_q)) { _actId = /\b(dip nha|nhap trach|don nha|move house|relocat)\b/.test(_q) ? 'move' : 'build'; _actLabel = /\b(dip nha|nhap trach|don nha|move house|relocat)\b/.test(_q) ? 'Dọn nhà' : 'Động thổ'; }
  else if (/\b(di xa|xuat hanh|du lich|cong tac|travel|trip|journey)\b/.test(_q)) { _actId = 'travel'; _actLabel = 'Xuất hành'; }
  else if (/\b(ky hop dong|ky ket|giao dich|sign contract|sign deal)\b/.test(_q)) { _actId = 'sign'; _actLabel = 'Ký kết'; }
