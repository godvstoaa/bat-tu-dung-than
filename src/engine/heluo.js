...
// ---- 河图洛书 口诀 (nền móng河洛理数, 《周易·系辞》) ----
// [loop 1229] Nguồn: Wikisource 河图原 + Wikipedia + 百度百科 + 知乎.
export const HELU_KOUJUE = {
  hetu: '«天一生水，地六成之；地二生火，天七成之；天三生木，地八成之；地四生金，天九成之；天五生土，地十成之» — ngũ hành sinh-thành số (1+6水/2+7火/3+8木/4+9金/5+10土).',
  hetuShort: '«一六共宗(水)，二七同道(火)，三八为朋(木)，四九为友(金)，五十共守(土)».',
  luoshu: '«戴九履一，左三右七，二四为肩，六八为足，五居中央» — cửu cung [4-9-2 / 3-5-7 / 8-1-6], 纵/横/斜 各 hoà = 15.',
  origin: '«Hà xuất đồ, Lạc xuất thư, thánh nhân tắc chi» (周易·系辞) — số trời 1+3+5+7+9=25, đất 2+4+6+8+10=30, tổng 55.',
};
...
const HEX64 = {
  ...
  // [loop 550 FIX] 8 cặp (16 entry) bị ĐẢO thượng/hạ từ trước → sai quẻ + 卦辞/爻辞/后天卦.
  //   Key = thượng+hạ (xác nhận qua 巽艮=渐#53). Nay sửa theo định nghĩa tên quẻ Dịch.
  //   坎上坤下=比#8 (không phải师), 坎上震下=屯#3 (không phải解), v.v.
  '坎坤': ['比', 'Tỷ', 8], '坤坎': ['师', 'Sư', 7], '震坎': ['解', 'Giải', 40], '坎震': ['屯', 'Truân', 3],
  '巽坎': ['涣', 'Hoán', 59], '坎巽': ['井', 'Tỉnh', 48], '离坎': ['未济', 'Vị Tế', 64], '坎离': ['既济', 'Ký Tế', 63],
  '艮坎': ['蒙', 'Mông', 4], '坎艮': ['蹇', 'Kiển', 39], '兑坎': ['困', 'Khốn', 47], '坎兑': ['节', 'Tiết', 60],
  '乾坎': ['讼', 'Tụng', 6], '坎乾': ['需', 'Nhu', 5], '震坤': ['豫', 'Dự', 16], '坤震': ['复', 'Phục', 24],
  '巽坤': ['观', 'Quán', 20], '坤巽': ['升', 'Thăng', 46], '离坤': ['晋', 'Tấn', 35], '坤离': ['明夷', 'Di Minh', 36],
  '艮坤': ['剥', 'Bác', 23], '坤艮': ['谦', 'Khiêm', 15], '兑坤': ['萃', 'Tụy', 45], '坤兑': ['临', 'Lâm', 19],
  '乾坤': ['否', 'Bĩ', 12], '坤乾': ['泰', 'Thái', 11], '巽震': ['益', 'Ích', 42], '震巽': ['恒', 'Hằng', 32],
  '离震': ['噬嗑', 'Thệ Hạp', 21], '震离': ['丰', 'Phong', 55], '艮震': ['颐', 'Di', 27], '震艮': ['小过', 'Tiểu Quá', 62],
  '兑震': ['随', 'Tùy', 17], '震兑': ['归妹', 'Quy Muội', 54], '乾震': ['无妄', 'Vô Vọng', 25], '震乾': ['大壮', 'Đại Tráng', 34],
...
function flyBranches(lines, isYangHour) {
  ...
  if (N >= 1 && N <= 5) {
    ...
    // [loop 550 FIX] N=0 XẢY RA thực tế: quẻ thuần (乾 6 dương / 坤 6 âm) + giờ NGƯỢC cực
    //   (vd 坤 6 âm + giờ dương 子-巳 → N=0). Trước đây rơi khỏi mọi nhánh → mapping=[0...0]
    //   → yuantang.line=0 (vô nghĩa), 后天卦=本命卦. Nay gộp vào nhánh纯爻 DISPUTED.
  } else if (N === 6 || N === 0) {
