  ...
  try {
    ...
    el.innerHTML = `
      <div style="display:flex;flex-wrap:wrap;gap:8px;align-items:center">
        ...
        <span class="zh big">${esc(dayGz)}</span> ${(() => { const n = ganZhiNayin(dayGz); return n ? `<span class="hint-inline" title="${esc(nayinInfo(n)?.meaning || '')}">${esc(n)}</span>` : ''; })()}${lr?.dayStage && STAGE_VI[lr.dayStage] ? ` <span class="hint" style="font-size:.75em" title="十二长生 — sinh khí ngày">🌱 ${esc(STAGE_VI[lr.dayStage])}</span>` : ''}
