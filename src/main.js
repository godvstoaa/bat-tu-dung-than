...
function renderQuickSummary() {
  ...
  const rows = [
    { icon: '🧬', label: 'Mệnh bạn', text: (() => { let t = `${pattern.vi || '?'}, thân ${c.strength?.strong ? 'vượng (mạnh)' : 'nhược (yếu)'}. <b>Điểm tổng mệnh: ${syn.score ?? '?'}/100 (${syn.gradeVi ?? '?'})</b>`; try { const lv = classifyChartLevel(c); if (lv.levelVi) t += ` · <b style="color:var(--gold-bright)">${lv.levelVi}</b>`; } catch (e) {} return t + '.'; })() },
