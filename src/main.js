...
function renderLiuRi(dateStr) {
  ...
  $('liuri').innerHTML = `
    ...
    <div class="ly-head"><span class="zh big">${esc(r.ganZhi)}</span> ${esc(r.solar)} · can <b>${esc(GOD_VI[r.ganGod] || r.ganGod)}</b> → <span class="ln-rate ${cls}">${esc(r.rating)} (${esc(String(r.score))}/100)</span>${r.dayStage ? ` <span class="hint" style="font-size:.8em" title="十二长生 sinh khí ngày">🌿${esc(r.dayStage)}</span>` : ''}${r.dayPillarStrength ? ` <span class="hint" style="font-size:.8em" title="盖头截脚 trụ ngày">${esc(r.dayPillarStrength.split(' ')[0])}</span>` : ''}</div>
